...
void calculate_implicitly(struct BiosensorInformation *bio_info)
{
  int a; 	
  ...
  pS = (double *) malloc((point_count - 1) * sizeof(double));
  qS = (double *) malloc((point_count - 1) * sizeof(double));
  pP = (double *) malloc((point_count - 1) * sizeof(double));
  qP = (double *) malloc((point_count - 1) * sizeof(double));  
	  ...
	  if (!is_boundary)
	  {
		...
		if (enz_layer)
		  kinetics_part = dt * ((v_max * last_s[a]) / 
            (km * (prod_inh?(1 + last_p[a] / kp):1) + last_s[a] * (subs_inh?(1 + last_s[a] / ks):1)));	
